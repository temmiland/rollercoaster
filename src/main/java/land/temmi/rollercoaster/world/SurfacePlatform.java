...
/**
 * An ordinary tile map placed as one walking plane of a room.
 *
 ...
 * mirrors a map.
 */
public final class SurfacePlatform {
    ...
    /** {@code x}, {@code y} and {@code z} are the room tile occupied by local tile (0, 0). */
    public SurfacePlatform(String id, GravityState gravity, LoadedMap map, int x, int y, int z) {
        if (id == null || gravity == null || map == null) throw new IllegalArgumentException("Platform data required");
        this.id = id;
    ...
    /** Room tile holding local tile {@code (u, v)}. */
    ...
    /** Whether the plane's own terrain rules allow a step between two of its room tiles. */
    ...
    /** Centre of the walkable surface of a room tile, lifted out of the plane by its terrain height. */
    ...
    public Matrix4 transform(Matrix4 out) {
        ...
        // A tile map's tile (u, v) spans local [u-1, u]; its centre has to land on the room tile.
